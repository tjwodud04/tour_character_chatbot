  ...
  async startRecording(){
    try{
      ...
      if(!MediaRecorder.isTypeSupported(mimeType)){
        alert('이 브라우저는 "audio/webm;codecs=opus" 녹음을 지원하지 않아요. 크롬/엣지 최신 버전을 사용해 주세요.');
        return false;
      }
...
function updateLipSync(){
  ...
  const d=audioManager.getAudioData();
  if(!d?.length) return;
  let sum=0; for(let i=0;i<d.length;i++) sum+=Math.abs(d[i]-128);
  const openY=(sum/d.length)/128;
  live2dManager.updateLipSync(Math.min(1, Math.max(0, openY)));
}
...
// 공용 카테고리 라벨 (중복 제거: token/meta 처리 및 카드 렌더 공용)
...
async function handleRecording(){
  ...
  if(!audioManager.isRecording){
    ...
    try{
      ...
      // === 무음 대기 멘트: 약 3.5초 후 1회 출력 (TTS 사용 안 함) ===
              ...
              async () => {
                ...
                try {
                  ...
                  if (!j.courses || j.courses.length === 0) {
                    ...
                    // 서버가 내려준 멘트(두 번째 답변) 출력
                    const say = j.say_text || '추천 코스를 가져왔어!';
                    chatManager.addMessage('ai', say);

                    // ▼ gpt-4o-mini-tts 오디오 재생 + Live2D 립싱크
                    if (j.audio) {
                      try {
                        chatManager.isPlaying = true;
                        live2dManager.setExpression('speaking');
                        await live2dManager.playAudioWithLipSync(j.audio);
                      } catch (e) {
                        console.error('코스 멘트 재생 실패:', e);
                      } finally {
...
async function sendAudioToServerStream(audioBlob, characterType='kei'){
  ...
  const clearInterim = ()=>{ if(interimTimer1) clearTimeout(interimTimer1); if(interimTimer2) clearTimeout(interimTimer2); interimTimer1=interimTimer2=null; interimScheduled=false; };
  let interimText1='';
  const catLabel = (c)=> CAT1_NAME[c] || '관광지';
...
// Sanitizer & 추천 카드 렌더
// 허용 태그: <a>, <br>
// 허용 속성(a): href, target, rel  (그 외 모두 제거)
function _esc(s){
  const map = {
    '&': '&amp;',
    '<': '&lt;',
